...
std::string DataServer::get_timestamp(std::string &key, std::string &curr_class, uint32_t conf_id){
    ...
    if(curr_class == CAS_PROTOCOL_NAME){
        result = CAS.get_timestamp(key, conf_id, cache, persistent, lock);
...
std::string DataServer::reconfig_finalize(std::string &key, std::string &timestamp, std::string &curr_class, uint32_t conf_id){
    std::string result;
    if(curr_class == CAS_PROTOCOL_NAME){
        result = CAS.get(key, conf_id, timestamp, cache, persistent, lock);
    }else{
        //Error scenario;
    }
    return result;
...
//        }
        printf("%s", bbuf);
        result = CAS.put(key, conf_id, value, timestamp, cache, persistent, lock);
        result = CAS.put_fin(key, conf_id, timestamp, cache, persistent, lock);
    }else if(curr_class == ABD_PROTOCOL_NAME){
...
std::string DataServer::put(std::string &key, std::string &value, std::string &timestamp, std::string &curr_class, uint32_t conf_id){
    std::string result;
    if(curr_class == CAS_PROTOCOL_NAME){
        char bbuf[1024*128];
        int bbuf_i = 0;
