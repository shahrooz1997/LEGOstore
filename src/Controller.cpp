...
Controller::Controller(uint32_t retry,
                       uint32_t metadata_timeout,
                       uint32_t timeout_per_req,
                       const string &detacenters_info_file,
                       const string &workload_file,
                       const string &placements_file) {
  ...
  properties.local_datacenter_id = 7;
  ...
  reconfigurer_p = unique_ptr<Reconfig>(new Reconfig(0,
                                                     properties.local_datacenter_id,
                                                     retry,
                                                     metadata_timeout,
                                                     timeout_per_req,
...
//            it["SLO_write"].get_to(gwkl->slo_write);
      ...
      if (it.contains("excluded_dcs")){
        it["excluded_dcs"].get_to(grp.excluded_dcs);
      }
...
//        element["m"].get_to(grp.placement.m); // m should show the number of datacenters instead of the number of servers in the current placement
    ...
    grp.placement.f = grp.availability_target;
...
int execute(const char *command, const vector<string> &args, string &output) {
  ...
  if (pid == -1) {
    ...
    while ((dup2(pipedes[1], STDOUT_FILENO) == -1)
        && (errno == EINTR)) {} // we used while so it will happen even if an interrupt arises
    while ((dup2(pipedes[1], STDERR_FILENO) == -1) && (errno == EINTR)) {}
    close(pipedes[1]);
    close(pipedes[0]);
    switch (args.size()) {
      case 1:execl(command, args[0].c_str(), (char *) 0);
        break;

      case 2:execl(command, args[0].c_str(), args[1].c_str(), (char *) 0);
        break;

      case 3:execl(command, args[0].c_str(), args[1].c_str(), args[2].c_str(), (char *) 0);
        break;

      case 4:execl(command, args[0].c_str(), args[1].c_str(), args[2].c_str(), args[3].c_str(), (char *) 0);
        break;
...
int Controller::kill_datacenter(uint32_t datacenter_id) {
  string command = "killall Server";
  ...
  DPRINTF(DEBUG_RECONFIG_CONTROL,
          "Killing server with ip %s\n",
          properties.datacenters[datacenter_indx]->servers[0]->ip.c_str());
  vector<string> args =
      {"ssh", "-o", "StrictHostKeyChecking no", "-t", properties.datacenters[datacenter_indx]->servers[0]->ip, command};
  string output;
  int status = execute("/usr/bin/ssh", args, output);
...
//            DPRINTF(DEBUG_RECONFIG_CONTROL, "aaaa %s\n", master.prp.datacenters[j]->metadata_server_ip.c_str());
      if (properties.group_configs[group_config_i].groups[i].client_dist[j] != 0) {
        clients_thread.emplace_back(&Controller::run_client,
                                    this,
                                    properties.datacenters[j]->id,
                                    properties.group_configs[group_config_i].id,
