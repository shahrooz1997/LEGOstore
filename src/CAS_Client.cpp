...
int failure_support_optimized(const string &operation,
  ...
  EASY_LOG_INIT_M(string("to do ") + operation + " on key " + key + " with quorum size " + to_string(quorom.size()),
                  DEBUG_CAS_Client);
...
int CAS_Client::get_timestamp(const string &key, unique_ptr<Timestamp> &timestamp_p, bool is_put) {
  ...
  if (!is_put && p.quorums[this->local_datacenter_id].Q1.size() < p.quorums[this->local_datacenter_id].Q4.size()) {
    op_status = CAS_helper::failure_support_optimized("get_timestamp",
                                                      key,
                                                      "",
                                                      vector<string>(p.m, ""),
                                                      this->retry_attempts,
                                                      p.quorums[this->local_datacenter_id].Q4,
                                                      p.servers,
                                                      p.m,
                                                      this->datacenters,
                                                      this->current_class,
    ...
    op_status = CAS_helper::failure_support_optimized("get_timestamp",
                                                      ...
                                                      vector<string>(p.m, ""),
                                                      this->retry_attempts,
                                                      p.quorums[this->local_datacenter_id].Q1,
                                                      p.servers,
                                                      p.m,
                                                      this->datacenters,
                                                      this->current_class,
...
//    EASY_LOG_M("lock for the key granted");
  ...
  DPRINTF(DEBUG_CAS_Client,
          "latencies%d: %lu\n",
          le_counter++,
          time_point_cast<chrono::milliseconds>(chrono::system_clock::now()).time_since_epoch().count() - le_init);
...
//    fflush(stdout);
  ...
  DPRINTF(DEBUG_CAS_Client, "calling failure_support_optimized.\n");
  op_status = CAS_helper::failure_support_optimized("put",
                                                    key,
                                                    timestamp_p->get_string(),
                                                    chunks,
                                                    this->retry_attempts,
                                                    p.quorums[this->local_datacenter_id].Q2,
                                                    p.servers,
                                                    p.m,
                                                    this->datacenters,
                                                    this->current_class,
...
//            return S_RECFG;
  ...
  op_status = CAS_helper::failure_support_optimized("put_fin",
                                                    key,
                                                    timestamp_p->get_string(),
                                                    vector<string>(p.m, ""),
                                                    this->retry_attempts,
                                                    p.quorums[this->local_datacenter_id].Q3,
                                                    p.servers,
                                                    p.m,
                                                    this->datacenters,
                                                    this->current_class,
