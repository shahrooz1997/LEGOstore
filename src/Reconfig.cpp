...
void _send_one_server(const string operation, promise<strVec> &&prm, const string key,
  ...
  while (!success && retry_times > 0) {
    ...
    if (!c.is_connected()) {
      DPRINTF(DEBUG_CAS_Client, "!c.is_connected()\n");
      EASY_LOG_M(string("!c.is_connected() "));
...
//    }
  ...
  if (!success) {
    data.clear();
    DPRINTF(DEBUG_CAS_Client, "response failed\n");
 ...
 */
int failure_support_optimized(const string &operation,
                              const string &key,
                              const string &timestamp,
                              const vector<string> &values,
                              uint32_t RAs,
                              uint32_t number_to_respond,
                              vector<uint32_t> servers,
                              uint32_t total_num_servers,
                              vector<DC *> &datacenters,
                              const string current_class,
                              const uint32_t conf_id,
                              uint32_t timeout_per_request,
                              vector<strVec> &ret) {
  DPRINTF(DEBUG_CAS_Client, "started.\n");
  EASY_LOG_INIT_M(string("to do ") + operation + " on key " + key + " with quorum size " + to_string(number_to_respond),
                  DEBUG_CAS_Client);
...
Reconfig::Reconfig(uint32_t id, uint32_t local_datacenter_id, uint32_t retry_attempts, uint32_t metadata_server_timeout,
                   uint32_t timeout_per_request, vector<DC *> &datacenters) : Client(id,
                                                                                     local_datacenter_id,
                                                                                     retry_attempts,
...
int Reconfig::reconfig_one_key(const string &key,
                               const Group &old_config,
                               uint32_t old_conf_id,
                               const Group &new_config,
...
int Reconfig::reconfig(const Group &old_config, uint32_t old_conf_id, const Group &new_config, uint32_t new_conf_id) {
  ...
  for (auto it = old_config.keys.begin(); it != old_config.keys.end(); it++) {
    rets.emplace_back(async(launch::async,
                            &Reconfig::reconfig_one_key,
                            this,
                            *it,
                            old_config,
                            old_conf_id,
...
int Reconfig::update_one_metadata_server(const std::string &metadata_server_ip,
                                         uint32_t metadata_server_port,
                                         const std::string &key,
                                         uint32_t old_confid_id,
                                         uint32_t new_confid_id,
...
int Reconfig::update_metadata_info(const string &key,
                                   uint32_t old_confid_id,
                                   uint32_t new_confid_id,
                                   const string &timestamp,
                                   const Placement &p) {
  ...
  for (uint k = 0; k < datacenters.size(); k++) {
    rets.emplace_back(async(launch::async,
                            &Reconfig::update_one_metadata_server,
                            this,
                            datacenters[k]->metadata_server_ip,
                            datacenters[k]->metadata_server_port,
                            key,
                            old_confid_id,
                            new_confid_id,
                            timestamp,
...
int Reconfig::send_reconfig_query(const Group &old_config,
                                  uint32_t old_conf_id,
                                  const string &key,
                                  unique_ptr<Timestamp> &ret_ts,
                                  string &ret_v) {
  ...
  if (old_config.placement.protocol == ABD_PROTOCOL_NAME) {
    ...
    op_status = ABD_helper_recon::failure_support_optimized("reconfig_query",
                                                            key,
                                                            "",
                                                            "",
                                                            this->retry_attempts,
                                                            old_config.placement.servers.size()
                                                                - old_config.placement.quorums[this->local_datacenter_id].Q2.size()
                                                                + 1,
    ...
    op_status = CAS_helper_recon::failure_support_optimized("reconfig_query",
                                                            key,
                                                            "",
                                                            vector<string>(old_config.placement.m, ""),
                                                            this->retry_attempts,
                                                            max(old_config.placement.servers.size()
                                                                    - old_config.placement.quorums[this->local_datacenter_id].Q3.size()
                                                                    + 1,
                                                                old_config.placement.servers.size()
                                                                    - old_config.placement.quorums[this->local_datacenter_id].Q4.size()
                                                                    + 1),
                                                            old_config.placement.servers,
                                                            old_config.placement.m,
                                                            this->datacenters,
...
int Reconfig::send_reconfig_finalize(const Group &old_config,
                                     uint32_t old_conf_id,
                                     const string &key,
  ...
  EASY_LOG_M("calling failure_support_optimized.");
  op_status = CAS_helper_recon::failure_support_optimized("reconfig_finalize",
                                                          key,
                                                          ts->get_string(),
                                                          vector<string>(old_config.placement.m, ""),
                                                          this->retry_attempts,
...
int Reconfig::send_reconfig_write(const Group &new_config,
                                  uint32_t new_conf_id,
                                  const string &key,
                                  unique_ptr<Timestamp> &ts,
                                  const string &value) {
  ...
  if (new_config.placement.protocol == ABD_PROTOCOL_NAME) {
    DPRINTF(DEBUG_ABD_Client, "calling failure_support_optimized.\n");
    EASY_LOG_M("calling failure_support_optimized.");
    op_status = ABD_helper_recon::failure_support_optimized("reconfig_write",
                                                            key,
                                                            ts->get_string(),
                                                            value,
                                                            this->retry_attempts,
                                                            new_config.placement.quorums[this->local_datacenter_id].Q2.size(),
                                                            new_config.placement.servers,
                                                            new_config.placement.m,
                                                            this->datacenters,
                                                            new_config.placement.protocol,
                                                            new_conf_id,
                                                            this->timeout_per_request,
                                                            ret);
...
//                DPRINTF(DEBUG_ABD_Client, "OK received for key : %s\n", key.c_str());
    ...
    assert(value.size() > 3);
    EASY_LOG_M(
        "calling liberasure.encode for value " + value.substr(0, 3) + "...[" + to_string(value.size()) + " bytes]");
    assert(this->liberasure.encode(value, chunks, new_config.placement.m, new_config.placement.k) == 0);

    EASY_LOG_M("calling failure_support_optimized.");
    DPRINTF(DEBUG_CAS_Client, "calling failure_support_optimized.\n");
    op_status = CAS_helper_recon::failure_support_optimized("reconfig_write",
                                                            key,
                                                            ts->get_string(),
                                                            chunks,
                                                            this->retry_attempts,
                                                            max(new_config.placement.quorums[this->local_datacenter_id].Q2.size(),
                                                                new_config.placement.quorums[this->local_datacenter_id].Q3.size()),
                                                            new_config.placement.servers,
                                                            new_config.placement.m,
                                                            this->datacenters,
                                                            new_config.placement.protocol,
...
int Reconfig::send_reconfig_finish(const Group &old_config,
                                   uint32_t old_conf_id,
                                   uint32_t new_conf_id,
                                   const string &key,
                                   unique_ptr<Timestamp> &ts) {
  ...
  if (old_config.placement.protocol == ABD_PROTOCOL_NAME) {
    DPRINTF(DEBUG_ABD_Client, "calling failure_support_optimized.\n");
    EASY_LOG_M("calling failure_support_optimized.");
    op_status = ABD_helper_recon::failure_support_optimized("finish_reconfig",
                                                            key,
                                                            ts->get_string(),
                                                            to_string(new_conf_id),
                                                            this->retry_attempts,
                                                            old_config.placement.servers.size()
                                                                - old_config.placement.f,
                                                            old_config.placement.servers,
                                                            old_config.placement.m,
                                                            this->datacenters,
                                                            old_config.placement.protocol,
...
//                DPRINTF(DEBUG_ABD_Client, "OK received for key : %s\n", key.c_str());
    ...
    DPRINTF(DEBUG_CAS_Client, "calling failure_support_optimized.\n");
    EASY_LOG_M("calling failure_support_optimized.");
    op_status = CAS_helper_recon::failure_support_optimized("finish_reconfig",
                                                            key,
                                                            ts->get_string(),
                                                            vector<string>(old_config.placement.m,
                                                                           to_string(new_conf_id)),
